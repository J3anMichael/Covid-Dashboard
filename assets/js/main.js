...
(() => {
    ...
    (async () => {
        ...
        if (response[1].status == "fullFilled") {
...
function loadCountries(data) {
    ...
    data.sort((a, b) => {                     //ORDENANDO OS PAISES
    ...
    for (index in data) {
        combo.options[combo.options.length] = new Option(
            data[index].Country,
            data[index].Country,
            data[index].Country === "Brazil" ? true : false, //VALOR DEFAULT
...
function handleChange() {
    ...
    if (country != "Global") {
        ...
        ).then(response => response.json()).then((json) => loadData(json));
...
function loadData(data) {
    ...
    let yCRecoveredDelta = data[1].Recovered - data[0].Recovered;
    ...
    let tCRecoveredDelta = data[2].Recovered - data[1].Recovered;
    ...
    insertDailyData("trecovered", tCRecoveredDelta, yCRecoveredDelta < tRecoveredDelta)
